...
namespace Slave
{
    ...
    public class Slave
    {
        ...
        private int previuosStepBonuses;
        ...
        {
            int currentBonuses = level.Player.TotalAttack + level.Player.TotalDefence - level.Player.Attack - level.Player.Defence;
            if (previuosStepBonuses > currentBonuses && currentBonuses != 0)
                //isNewObjective = true;
                //map = new Map(ref level);
                //map.SearchPath(location);
                //previuosStepBonuses = currentBonuses;
                if (SlaveObjectives.Peek() != objectives[(int)SlaveActions.FindBestEquip])
                {
                    isNewObjective = true;
                    map = new Map(ref level);
            ...
            Turn turn;
            if (!isNewObjective && SlaveObjectives.Peek() != objectives[(int)SlaveActions.MakeYourselfStronger] && SlaveObjectives.Peek() != objectives[(int)SlaveActions.GoHealYourself])
            {
